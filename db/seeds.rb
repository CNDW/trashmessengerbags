...
options_hash.each do |type, options|
	...
	options.each do |option|
		if option.length > 1
			Option.create( name: option[0], price: Random.rand(300), option_type: OptionType.find_by(name: type), images: [Image.create(title: option[0], image_data: File.open(Rails.root.join("db", "imageseed", "colors", "#{option[1]}")))])
		else
