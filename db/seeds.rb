...
options_hash.each do |type, options|
	...
	options.each do |option|
		...
		Image.create( title: option[0], image_data: File.open( Rails.root.join("db", "imageseed", "#{option[1]}") ) ) if option.length > 1
	end
end
